...
void Adafruit_D1_SSD1306::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
Adafruit_D1_SSD1306::Adafruit_D1_SSD1306(int8_t SID, int8_t SCLK, int8_t DC, int8_t RST, int8_t CS) : Adafruit_GFX(SSD1306_LCDWIDTH, SSD1306_LCDHEIGHT) {
  cs = CS;
  rst = RST;
  dc = DC;
  sclk = SCLK;
...
Adafruit_D1_SSD1306::Adafruit_D1_SSD1306(int8_t DC, int8_t RST, int8_t CS) : Adafruit_GFX(SSD1306_LCDWIDTH, SSD1306_LCDHEIGHT) {
  dc = DC;
  rst = RST;
  cs = CS;
  hwSPI = true;
...
Adafruit_D1_SSD1306::Adafruit_D1_SSD1306(int8_t reset) :
...
void Adafruit_D1_SSD1306::begin(uint8_t vccstate, uint8_t i2caddr, bool reset) {
  _vccstate = vccstate;
  _i2caddr = i2caddr;
...
void Adafruit_D1_SSD1306::invertDisplay(uint8_t i) {
  if (i) {
...
void Adafruit_D1_SSD1306::ssd1306_command(uint8_t c) {
  if (sid != -1)
  {
    // SPI
#ifdef HAVE_PORTREG
...
void Adafruit_D1_SSD1306::startscrollright(uint8_t start, uint8_t stop){
...
// Hint, the display is 16 rows tall. To scroll the whole display, run:
// display.scrollright(0x00, 0x0F)
void Adafruit_D1_SSD1306::startscrollleft(uint8_t start, uint8_t stop){
...
// Hint, the display is 16 rows tall. To scroll the whole display, run:
// display.scrollright(0x00, 0x0F)
void Adafruit_D1_SSD1306::startscrolldiagright(uint8_t start, uint8_t stop){
...
// Hint, the display is 16 rows tall. To scroll the whole display, run:
// display.scrollright(0x00, 0x0F)
void Adafruit_D1_SSD1306::startscrolldiagleft(uint8_t start, uint8_t stop){
  ssd1306_command(SSD1306_SET_VERTICAL_SCROLL_AREA);
  ssd1306_command(0x00);
  ssd1306_command(SSD1306_LCDHEIGHT);
...
void Adafruit_D1_SSD1306::stopscroll(void){
  ssd1306_command(SSD1306_DEACTIVATE_SCROLL);
}
...
void Adafruit_D1_SSD1306::dim(boolean dim) {
  uint8_t contrast;
...
void Adafruit_D1_SSD1306::display(void) {
...
// clear everything
void Adafruit_D1_SSD1306::clearDisplay(void) {
  memset(buffer, 0, (SSD1306_LCDWIDTH*SSD1306_LCDHEIGHT/8));
}


inline void Adafruit_D1_SSD1306::fastSPIwrite(uint8_t d) {
...
void Adafruit_D1_SSD1306::drawFastHLine(int16_t x, int16_t y, int16_t w, uint16_t color) {
  boolean bSwap = false;
  switch(rotation) {
    case 0:
...
void Adafruit_D1_SSD1306::drawFastHLineInternal(int16_t x, int16_t y, int16_t w, uint16_t color) {
...
void Adafruit_D1_SSD1306::drawFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color) {
  bool bSwap = false;
  switch(rotation) {
    case 0:
      break;
...
void Adafruit_D1_SSD1306::drawFastVLineInternal(int16_t x, int16_t __y, int16_t __h, uint16_t color) {
